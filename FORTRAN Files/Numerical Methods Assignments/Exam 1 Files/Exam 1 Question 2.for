     ...
     &the zero has been found after", iterat, "iterations."
      ...
      do while(tollo.GE.tol) ! Main while loop for FALSE-POSITION
          olde = newe
          tollo = tmptol(newe,olde,c(1),c(2),c(3),c(4),c(5),cnstnt)
       ...
       ! -- Begin diagnostic troubleshooting code --
       ...
       ! I think the error has to do with the fact that the right bound is so large, it's messing with the
       ! error calculation. Perhaps I need a way of storing the previous estimation then calculating error using
       ! new vs old. Good luck!
       
       ! -- End diagnostic troubleshooting code --
     ...
     &+ c1*(x) + con
      ...
      seca = (p2 - (yp2*((p2-p1)/(yp2-yp1)))) ! The x value of the C point
      END FUNCTION